...
charging_stations$chargedevicestatus <- as.factor(charging_stations$chargedevicestatus)

### Set connector count to factor
charging_stations$connectorcount <- as.factor(charging_stations$connectorcount)
...
columns <- c("chargedeviceid",
             ...
             "dateupdated",
             "connectorcount")
charging_stations_subset <- charging_stations[, ..columns]

## Descriptive statistics

### Summary
summary(charging_stations_subset)

stargazer::stargazer(charging_stations_subset,
                     table.placement = "H",
                     digit.separator = "",
                     flip = TRUE,
                     header = FALSE,
                     label = "tab:charging_stations_numerical_summary_statistics",
                     title = "Charging Stations (numerical) summary statistics")

## Create plots

### charging stations over time: timeseries
ggplot2::ggplot(charging_stations_subset) +
